...
export const I = {
  seaIronRules: [
    ...
    { n:5, zh:"止损 = 权利金的50%，无条件出场", en:"Stop = 50% of premium paid: unconditional exit", critical:false },
    { n:6, zh:"日亏 ≥ 2× 单笔止损则停止交易", en:"Daily loss ≥ 2× single stop → stop trading", critical:false },
  ...
  checklist: [
    ...
    { zh:"失效位已写清：止损为入场权利金的50%，具体出场价已确定。", en:"Invalidation written: stop = 50% of entry premium, exact exit price defined." },
    ...
    { zh:"未触发熔断：当日亏损 < 2× 单笔止损 / 未连亏2笔。", en:"No circuit breakers triggered: daily loss < 2× single stop, consecutive losses < 2." },
  ...
  questions: [
    ...
    { cat:{zh:"止损",en:"Stop Loss"}, q:{zh:"期权开仓后已亏入场权利金的45%，接近50%止损线，但感觉'快到支撑了'，应该？",en:"Your option is down 45% from entry premium, near the 50% stop. You think 'support is close.' What should you do?"}, opts:{zh:["再等一下，快到支撑了","到50%立刻无条件市价出场","把止损放宽到-60%","换合约继续持有"],en:["Wait a little — support is near","Hit 50% loss → unconditional market exit","Move stop to −60%","Switch contracts and keep holding"]}, a:1, exp:{zh:"权利金亏50%无条件市价出场，没有'再等等'。止损规则不给情绪留空间。期权时间价值持续流失，等待只会让止损线更快到达。",en:"50% premium stop is unconditional. No 'a little longer.' Stop rules leave no room for emotion. Theta keeps burning — waiting only accelerates the stop."} },
    ...
    { cat:{zh:"风控",en:"Risk Mgmt"}, q:{zh:"当日已亏损1.8× 单笔止损，又出现信号，距2×熔断线还差0.2×，应该？",en:"You've lost 1.8× your single stop today. Another setup appears. 0.2× left before the 2× daily circuit breaker."}, opts:{zh:["做这笔，还没到2×","停止交易，已接近熔断线","做小仓位试试","信号好可以做"],en:["Take it — haven't hit 2× yet","Stop — you're already at the circuit breaker threshold","Try with very small size","Good setup = fine to take"]}, a:1, exp:{zh:"接近熔断线就应停止，不是到了才停。且该信号的止损(50%权利金)本身就超过剩余空间，本就不该做。",en:"Stop well before the threshold, not only when reached. Also, the 50% stop on this trade would exceed remaining room — this trade should not be taken regardless."} },
    ...
    { cat:{zh:"风控计算",en:"Position Calc"}, q:{zh:"账户$1000，单笔风险2%（=$20），期权价$0.40，最多几张？",en:"Account $1,000. Per-trade risk 2% (=$20). Option price $0.40. Maximum contracts?"}, opts:{zh:["1张","2张","5张","随意"],en:["1 contract","2 contracts","5 contracts","Feel it out"]}, a:0, exp:{zh:"单笔风险预算 = $1000×2% = $20。50%止损意味着入场权利金上限 = $20÷50% = $40，即期权价≤$0.40才符合。$0.40×100 = $40，最多1张。账户越小，对期权价格要求越严格。",en:"Risk budget = $1,000×2% = $20. 50% stop means max premium = $20÷50% = $40, so option price ≤$0.40. $0.40×100 = $40 per contract → 1 contract max. The smaller the account, the stricter the price filter."} },
